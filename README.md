<div align="center">

```
███████╗██╗██╗     ███████╗ ██████╗ ███████╗███╗   ██╗
██╔════╝██║██║     ██╔════╝██╔════╝ ██╔════╝████╗  ██║
█████╗  ██║██║     █████╗  ██║  ███╗█████╗  ██╔██╗ ██║
██╔══╝  ██║██║     ██╔══╝  ██║   ██║██╔══╝  ██║╚██╗██║
██║     ██║███████╗███████╗╚██████╔╝███████╗██║ ╚████║
╚═╝     ╚═╝╚══════╝╚══════╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝
```

**Simple file generator from Markdown**

[![Version](https://img.shields.io/badge/version-0.1.2-blue.svg)](https://github.com/VOTRE-USERNAME/filegen/releases)
[![Python](https://img.shields.io/badge/python-3.8+-green.svg)](https://www.python.org/)
[![License](https://img.shields.io/badge/license-GPL-orange.svg)](LICENSE)
[![Platform](https://img.shields.io/badge/platform-linux%20%7C%20macos%20%7C%20windows-lightgrey.svg)]()

</div>

## FileGen

FileGen is a versatile tool that automatically creates files from Markdown documents and responses generated by ChatGPT. It simplifies and speeds up the production of structured content for development projects, documentation, or any workflow requiring organized files.

## Installation

### 1. Using the install script (Linux/macOS/Windows)

```bash
curl -sSL https://raw.githubusercontent.com/r-seize/FileGen/main/install.sh | bash
```

if you encounter an externally-managed-environment error, it means your system Python is managed by the OS. Use a virtual environment or pipx to install safely:

```bash
python3 -m venv ~/filegen-venv
source ~/filegen-venv/bin/activate
pip install https://github.com/r-seize/FileGen/archive/refs/tags/v0.1.2.tar.gz
```

### 2. Using pip directly from the release archive

```bash
pip3 install https://github.com/r-seize/FileGen/archive/refs/tags/v0.1.2.tar.gz
```

Same note as above regarding externally-managed-environment. Using a virtual environment is recommended if installing on system Python.

### 3. On Ubuntu/Debian using the .deb package

```bash
wget https://github.com/r-seize/FileGen/releases/download/v0.1.2/python3-filegen_0.1.2-1_all.deb
sudo dpkg -i python3-filegen_0.1.2-1_all.deb
```

## Usage
```bash
filegen <file.md>                    Create files from Markdown
filegen --chatgpt                    Create files from ChatGPT response (paste content)
filegen --raw                        Create files from raw tree structure (paste content)
filegen <file.md> -o <dir>           Create in specific directory
filegen <file.md> --preview          Preview without creating
filegen <file.md> --force            Overwrite existing files
filegen --help                       Show this help
filegen --version                    Show version
```

### Examples

**From Markdown file:**
```bash
filegen structure.md
filegen structure.md -o my-project
filegen structure.md --preview
```

**From ChatGPT response:**
```bash
filegen --chatgpt
# Then paste your ChatGPT response and press Ctrl+D (Linux/Mac) or Ctrl+Z (Windows)
```

**From raw tree structure:**
```bash
filegen --raw
# Then paste your tree structure:
# project/
# ├── src/
# │   ├── main.py
# │   └── utils.py
# └── README.md
# Press Ctrl+D (Linux/Mac) or Ctrl+Z (Windows) when done
```

## Uninstallation

### Linux/macOS

- If installed via `.deb`:

```bash
sudo dpkg -r python3-filegen
sudo apt-get autoremove
sudo rm -rf /usr/share/doc/python3-filegen
sudo rm -rf /usr/local/bin/filegen
sudo rm -rf /usr/lib/python3/dist-packages/filegen*
```

- If installed via `pip`:

```bash
pip3 uninstall filegen
```

- If installed via `pipx`:

```bash
pipx uninstall filegen
```

- If installed in a `virtual environment`:

```bash
rm -rf /path/to/filegen-venv
```

### Windows

- If installed via `pip`:

```bash
pip uninstall filegen
```

- If installed via `pipx`:

```bash
pipx uninstall filegen
```

- If installed in a `virtual environment`:

```bash
rd /s /q C:\path\to\filegen-venv
```

- Remove remaining binaries or files manually:

```bash
del C:\Users\<User>\AppData\Local\Programs\Python\PythonXX\Scripts\filegen.exe
rd /s /q C:\Users\<User>\AppData\Roaming\FileGen
```

## Features

| Feature | Status |
|---------|--------|
| Generate files from Markdown | ✅ Done |
| Integrate ChatGPT responses | ✅ Done |
| CLI for quick file generation | ✅ Done |
| Configurable templates | 🚧 Coming soon |
| Support for multiple output formats | 🚧 Coming soon |
| Automatic creation of files and folders from a ChatGPT conversation via an extension | 💡 Ideas |


## About

We’d like to thank everyone who contributed ideas, tested the tool, or provided feedback during development. Your support is greatly appreciated!

If you have suggestions, feature requests, or improvements, please don’t hesitate to open an issue or submit a pull request. Every contribution helps make FileGen better for everyone.